...
class ModelNet_Dataset(Dataset):
    def __init__(self, dataset, num_points, num_classes, dataset_dir, partition='train', 
                 noise_type="sym",noise_rate=0.0, select_indices=None):
        ...
        if self.dataset == 'ModelNet40':
            ...
            data_dir = os.path.join(self.dataset_dir, 'ModelNet10/%s_pt_feat_small.npy'%(self.partition))
            ori_label_dir = os.path.join(self.dataset_dir, 'ModelNet10/%s_ori_label.npy'%(self.partition))
            
            if partition=="train" and noise_rate>0:
                if noise_type=="sym":
                    label_dir = os.path.join(self.dataset_dir, 'ModelNet10/%s_label_%s.npy'%(self.partition, int(noise_rate*100)))
                else:
                    label_dir = os.path.join(self.dataset_dir, 'ModelNet10/%s_asy_label_%s.npy'%(self.partition, int(noise_rate*100)))
            ...
            self.label = np.load(label_dir)
            self.ori_label = np.load(ori_label_dir)
            correct_number = np.sum(self.label == self.ori_label)
            false_number = len(self.label) - correct_number
...
class MINIST3D(Dataset):
    def __init__(self, dataset, num_points, num_classes, dataset_dir, partition='train', 
        ...
        if partition == 'train':
            ...
            self.label = np.load('datasets/3D_MNIST/train_label_80.npy')
